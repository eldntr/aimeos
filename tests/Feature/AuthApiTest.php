...
class AuthApiTest extends \Tests\TestCase
{
    ...
    public function test_register_seller_via_api(): void
    {
        ...
        $unique = uniqid();
        $data = [
            'code' => "mocksiteapi-seller-{$unique}",
            'email' => "seller.api.{$unique}@example.com",
            'password' => 'password123',
            'password_confirmation' => 'password123',
            'name' => 'John Doe',
        ...
        $user = User::where('email', "seller.api.{$unique}@example.com")->first();
        $this->assertNotNull($user);
        ...
        $site = \Illuminate\Support\Facades\DB::table('mshop_locale_site')
            ->where('code', "mocksiteapi-seller-{$unique}")
            ->first();
    ...
    {
        $unique = uniqid();
        $email = "upgrade.test.{$unique}@example.com";
        $code = "mocksiteapi-upgrade-{$unique}";
    ...
    {
        $unique = uniqid();
        $email = "fail.upgrade.{$unique}@example.com";
        $code = "mocksiteapi-invpw-{$unique}";
